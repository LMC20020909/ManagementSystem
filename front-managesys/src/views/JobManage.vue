    ...
    <el-form style="margin-left: -40px" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
      <el-form-item label="字段：" prop="key">
        <el-select v-model="ruleForm.key" style="width: 160px;float: left" placeholder="请选择字段">
          <el-option label="部门名称" value="apartmentName"></el-option>
...
export default {
  ...
  methods:{
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios.get('http://localhost:8181/job/search',{params:_this.ruleForm}).then(function (resp) {
    ...
    page(currentPage){
      ...
      if(_this.ruleForm.value == ''){
        axios.get('http://localhost:8181/job/list/'+currentPage+'/'+_this.pageSize).then(function (resp) {
          _this.tableData = resp.data.data.data
          _this.total = resp.data.data.total
        })
      } else {
    ...
    edit(row){
      const _this = this
      _this.$router.push('/jobUpdate?id='+row.id)
    },
    del(row) {
      const _this = this
      this.$confirm('确认删除' + row.apartmentName + '部门的岗位' + '【'+row.name+'】吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        axios.delete('http://localhost:8181/job/deleteById/'+row.id).then(function (resp) {
          if(resp.data.code==0){
            _this.$alert(row.apartmentName + '部门的岗位' + '【' + row.name + '】已删除', '', {
              confirmButtonText: '确定',
              callback: action => {
                location.reload()
              }
  ...
  created() {
    const _this = this
    axios.get('http://localhost:8181/job/list/1/'+_this.pageSize).then(function (resp) {
      _this.tableData = resp.data.data.data
      _this.total = resp.data.data.total
    })
  }
