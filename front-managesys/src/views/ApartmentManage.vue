<template>
  <div style="margin-top: 60px;margin-left:80px;border: 0px solid red;" >
    <el-form style="margin-left: -40px" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
      <el-form-item label="字段：" prop="key">
        <el-select v-model="ruleForm.key" style="width: 160px;float: left" placeholder="请选择字段">
          ...
          <el-option label="部门负责人" value="master"></el-option>
          <el-option label="部门联系电话" value="telephone"></el-option>
        </el-select>
      </el-form-item>
    ...
    <el-table
        ...
        border
        stripe
        style="width:1151px;position: relative;top:-40px;">
      <el-table-column
          fixed
          prop="id"
          label="部门ID"
...
export default {
  ...
  methods:{
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios.get('http://localhost:8181/apartment/search',{params:_this.ruleForm}).then(function (resp) {
    ...
    page(currentPage){
      ...
      if(_this.ruleForm.value == ''){
        axios.get('http://localhost:8181/apartment/list/'+currentPage+'/'+_this.pageSize).then(function (resp) {
          _this.tableData = resp.data.data.data
          _this.total = resp.data.data.total
        })
      } else {
    ...
    edit(row){
      const _this = this
      _this.$router.push('/apartmentUpdate?id='+row.id)
    },
    del(row) {
      const _this = this
      this.$confirm('确认删除部门【'+row.name+'】吗？', '提示', {
        ...
        axios.delete('http://localhost:8181/apartment/deleteById/'+row.id).then(function (resp) {
          if(resp.data.code==0){
  ...
  created() {
    const _this = this
    axios.get('http://localhost:8181/apartment/list/1/'+_this.pageSize).then(function (resp) {
      _this.tableData = resp.data.data.data
      _this.total = resp.data.data.total
    })
  }
