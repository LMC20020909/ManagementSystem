...
import org.hustzl.mapper.MoveinMapper;
import org.hustzl.service.MoveinService;
 ...
 */
@Service
public class MoveinServiceImpl extends ServiceImpl<MoveinMapper, Movein> implements MoveinService {
    ...
    public void exportTest(OutputStream out, String excelTitle) throws IOException {
        ...
        String[] rowsName = new String[]{"id", "姓名", "岗位", "入职时间"};
        // 创建导出数据集合 后续会将dataList中的数据写到Excel
        List<Object[]> dataList = new ArrayList<>();
        // 从数据库查询用户列表
        QueryWrapper queryWrapper = new QueryWrapper();
        List<Movein> moveinList = moveinMapper.selectList(queryWrapper);
        // 将用户列表信息封装到一个Object数组
        // 我这里封装Object数组 是为了方便后续代码复用,不会将对象类型写死
        // 当然也可以在下一层使用反射来做,太麻烦了 还是这样转一下吧
        for(Movein movein: moveinList){
            //将数据库查到的每条数据 循环封装到Object[]
            Object[] objs = new Object[]{movein.getId(), movein.getEmployeeId(), movein.getJobId(), movein.getCreateDate()};
            //将转换好的数据 存入dataList
            dataList.add(objs);
        }
